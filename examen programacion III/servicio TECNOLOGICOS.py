...
def send_recovery_email(user_email, recovery_code):
    ...
    sender_password = "dsxx rzkk sddj lnoo"     # Cambia esto por tu contraseña o contraseña de aplicación
...
# Función para validar que solo se introduzcan letras y caracteres especiales en Nombres, Apellidos y Dirección
def validate_name_input(value):
    # Expresión regular que permite solo letras, espacios y caracteres especiales como ñ y acentos
...
# Función para validar el formato del correo electrónico con @
def is_valid_email(email):
    return re.match(r"[^@]+@[^@]+\.[^@]+", email)

# Función para permitir solo letras y caracteres especiales en Nombres, Apellidos y Dirección
def validate_name_input(char):
    return char.isalpha() or char in " áéíóúÁÉÍÓÚñÑ"       
##----------------------------------------------#
##----------------------------------------------#
...
def register_user():
    ...
    if not validate_name_input(first_name):
        messagebox.showerror("Error", "El nombre NO debe contener caracteres especiales \n solo debe contener letras y espacios.\nEjemplo:'Fabian Ernesto'")
        return
    
    # Validar los nombres y apellidos
    if not validate_name_input(last_name):
        messagebox.showerror("Error", "El apellido NO  debe contener caracteres especiales\n solo debe contener letras y espacios.\nEjemplo:'Benalcázar Andrango'")
        return
    ...
    if not user_id or not first_name or not last_name or not address or not password or not email:
        messagebox.showerror("Error", "Porfavor campos son obligatorios.")
    else:
...
def show_service_details(service_name):
    ...
    if service_name == "INSTALACION SERVIDOR INFORMATICO":
        description = "Un servidor Informatico es un sistema que proporciona recursos, datos, servicios o programas \n  a otros ordenadores, conocidos como clientes, a través de una red. \n  En teoría, se consideran servidores aquellos ordenadores que comparten recursos con máquinas cliente.\n  TELF: 0997000273  Tlgo. FABIAN BENALCAZAR "
    elif service_name == "INSTALACION CAMARAS DE SEGURIDAD":
        description = " !  SIENTASE SEGURO SU TRANQUILIDAD NO TIENE PRECIO   ! \n  Existen dos tipos de cámaras: analógicas y digitales.\n  telfono \n  0997000273 -Tlgo.FABIAN BENALCAZAR"
    elif service_name == "INSTALACION ALARMAS DE SEGURIDAD":
        description = "Proteja y monitoree su hogar  o negocio desde su celular instale siStema de seguridad electrónica con CyberMAS \n  TELF: 0997000273 \n Tlgo.FABIAN BENALCAZAR   .."
    elif service_name == "INSTALACIONES ELECTRICAS":
        description = " Es el conjunto de elementos tales como: tuberías, conductores, accesorios, dispositivos, entre otros, que tienen como objetivo SUMINISTRAR de energía eléctrica a la vivienda TENEMOS  proyectos  instalciones de trasformadores de altatencion \n TELF  0997000273 \n  Tlgo FABIAN BENALCAZAR "
    elif service_name == " HERRAMIENTAS":
        description = " sunistra toda clase de herramientas para tus proyectos , he infraestructura Electricas y Tecnologicas \n  TLF. 0997000273  \n  Tlgo. FABIAN BENALCAZAR    "
    elif service_name == "PROYECTOS DESARROLLO DE SOFTWARE":
        description = " El desarrollo de software es un proceso creativo y técnico que puede abarcar una gran variedad de productos, desde aplicaciones móviles hasta sistemas empresariales. El objetivo es convertir conceptos abstractos en software funcional que satisfaga las necesidades del usuaRIO \n     "
    else:
...
#img = Image.open(r'./dist/servidor.png')  # Usando ruta relativa
